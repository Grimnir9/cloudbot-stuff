...
link_insult = ["Pay Attention!", "Hey Dumb Ass!", "Wake Up!", "Numbnuts!", "Uhhh, duh!!"]
...
def format_reply(history):
    ...
    insult = random.choice(link_insult)
    if len(history) == 1:
        return "%s %s linked that %s ago." % (insult, last_nick, last_time)
    ...
    return "%s that url has been posted %s in the past %s by %s (%s)." % (insult, ordinal,
